...
function post_indexer_global_install() {
	...
	if (get_site_option( "post_indexer_installed" ) == "yes") {
		...
		$post_indexer_table1 = "CREATE TABLE IF NOT EXISTS `" . $wpdb->base_prefix . "site_posts` (
		  ...
		  `post_author_display_name` varchar(100),
		  `post_title` text,
		  `post_content` text,
		  ...
		  `post_hits` int(2) default NULL,
		  `post_popularity_score` int(2) default NULL,
  ...
  PRIMARY KEY  (`term_count_id`)
		...
		$post_indexer_table5 = "";
				...
				$post_indexer_table6 ="CREATE TABLE IF NOT EXISTS `wp_site_term_taxonomy` (
...
";
		...
		$wpdb->query( $post_indexer_table6 );
...
function post_indexer_post_insert_update($tmp_post_ID){
	...
	if ($tmp_post->post_type == 'page'){
		...
		$featured_image = post_image_url($tmp_post_ID);
		...
		(post_id, blog_id, site_id, sort_terms, post_author, post_author_display_name, post_title, post_content, post_content_stripped, post_permalink, post_featured_image, post_published_gmt, post_published_stamp, post_modified_gmt, post_modified_stamp, post_terms, blog_public, post_type)
		VALUES
		('" . $tmp_post_ID . "','" . $wpdb->blogid . "','" . $wpdb->siteid . "','" . $tmp_sort_terms . "','" . $tmp_post->post_author . "','" . $post_author_display_name . "','" . addslashes($tmp_post->post_title) . "','" . addslashes($tmp_post->post_content) . "','" . addslashes(post_indexer_strip_content($tmp_post->post_content)) . "','" . get_permalink($tmp_post_ID) . "','" . $featured_image . "','" . $tmp_post->post_date_gmt . "','" . strtotime($tmp_post->post_date_gmt) . "','" . $tmp_post->post_modified_gmt . "','" . time() . "','" . $tmp_post_terms . "','" . $tmp_blog_public . "','" . $tmp_post->post_type . "')");
		      ...
		      if ( is_array($existing_terms) && count($existing_terms) ) {
		        ...
		        foreach ($new_terms as $term) {
		          ...
		          if ( in_array($term->slug, $existing_slugs) ) {
		            ...
		            $p_slug = $wpdb->get_var( $wpdb->prepare( "SELECT slug FROM {$wpdb->terms} WHERE term_id = " . $term->parent ) );
					$p_siteterm = $wpdb->get_var( $wpdb->prepare( "SELECT term_id FROM {$wpdb->base_prefix}site_terms WHERE slug = '{$p_slug}'" ) );

		            //add taxonomy
		            $wpdb->insert( $wpdb->base_prefix . 'site_term_taxonomy', array( 'term_id' => $id, 'taxonomy' => $term->taxonomy, 'parent' => $p_siteterm) );
